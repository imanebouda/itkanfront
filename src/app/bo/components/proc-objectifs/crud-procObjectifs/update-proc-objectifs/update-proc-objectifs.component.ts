...
@Component({
  selector: 'app-update-proc-objectifs',
  templateUrl: './update-proc-objectifs.component.html',
  styleUrls: ['./update-proc-objectifs.component.scss']
})
export class UpdateProcObjectifsComponent {
...
constructor(
  ...
  this.updateClientForm = new FormGroup({
    ...
    ProcID :new FormControl(null, [Validators.required])
  });
}

ngOnInit(): void {
  const ProcID  =this.generalService.getProcedureID('list');
  ...
  this.updateClientForm.get('ProcID')?.setValue(ProcID)
}
...
CallAddApi() {
  ...
  this.procedureObjectifs_src.UpdateProcObjectifs(this.updateClientForm.value)
  .subscribe((r: any) => {
    r = JSON.parse(r);
