...
export class SiteComponent {
 ...
 SupprimerParametrage(site: any) {
     Swal.fire({
         title: 'Suppression',
         html: `<b>Êtes-vous sûr de vouloir supprimer site : </b></br>  ${site?.Libelle} ?`,
         icon: 'warning',
         ...
         if (result?.value) {
             ...
             this.is_loading = true;
             this.Supprimer = this.site_service
 ...
 ShowFormulaire(action: any, data_selected_parametrage: any = null) {
     if (action == 'Ajouté') {
         ...
         this.Header_info = `Modifier site : (${data_selected_parametrage?.Libelle})`;
         setTimeout(() => {
 ...
 /* ---------------- Férmeture du popup Ajout des sites ---------------- */
 ...
 /* ---------------- Férmeture du popup Update des sites ---------------- */
