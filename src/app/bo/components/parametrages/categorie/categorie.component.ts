...
export class CategorieComponent {
 ...
 constructor(
     ...
     private parametrage_service: CategorieService
 ) {
 ...
 SupprimerParametrage(categorie: any) {
     Swal.fire({
         title: 'Suppression',
         html: `<b>Êtes-vous sûr de vouloir supprimer parametrage : </b></br>  ${categorie?.Libelle} ?`,
         icon: 'warning',
 ...
 ShowFormulaire(action: any, data_selected_parametrage: any = null) {
     if (action == 'Ajouté') {
         ...
         this.Header_info = `Modifier catégorie : (${data_selected_parametrage?.Libelle})`;
         setTimeout(() => {
 ...
 /* ---------------- Férmeture du popup Ajout des categories ---------------- */
 ...
 /* ---------------- Férmeture du popup Update des categories ---------------- */
